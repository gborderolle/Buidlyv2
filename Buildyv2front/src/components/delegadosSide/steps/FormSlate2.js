...
const FormSlate2 = (props) => {
  ...
  const slateList1 = filteredSlateList.map((slate, index) => {
    // Buscar la URL de la imagen del candidato usando candidateId
    const candidate = reduxCandidateList.find(
      (candidate) => candidate.candidateId === slate.candidateId
    );

    const partyImageURL = slate.slateImageURL
      ? slate.slateImageURL
      : props.myPartyImageURL;
